...
1) полный текст (STT),
2) краткий конспект.
Также если отправить ссылку на YouTube — бот скачивает аудио дорожку видео, делает расшифровку через STT и отправляет только конспект (без полного текста).

Сделано через OpenAI-совместимый endpoint NeuroAPI: `https://neuroapi.host/v1`.

## Что умеет
- Принимает `voice`, `audio` и `document` (аудио).
- Принимает YouTube-ссылки (`youtube.com/watch`, `youtu.be`, `youtube.com/shorts/...`).
...
- Для YouTube-ссылок скачивает аудио дорожку видео и отправляет только `📌 Конспект видео`.
...
- `YOUTUBE_TRANSCRIPT_MAX_CHARS` — ограничение длины расшифровки перед отправкой в модель.
